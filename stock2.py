...
class Order(BaseModel):
    ...
    cart = models.ForeignKey(
        Cart, on_delete=models.CASCADE, related_name="orders", verbose_name=_("Cart")
    )
    total_price = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.00, verbose_name=_("Total Price")
    )
    ...
    discount_amount = models.DecimalField(
        max_digits=8, decimal_places=2, default=0.00, verbose_name=_("Discount Amount")
    )

    # Add this flag to prevent infinite recursion
    _calculating_totals = False
    ...
    def save(self, *args, **kwargs):
        ...
        # Remove the skip_totals parameter before calling parent save
        skip_totals = kwargs.pop('skip_totals', False)
        
        # تبدیل و گرد کردن مقادیر
        ...
        # به‌روزرسانی totals اگر OrderItemها وجود داشته باشند و skip_totals False باشد
        if not skip_totals and self.pk and self.orderitems.exists() and not self._calculating_totals: #type: ignore
            self.calculate_totals()

    def calculate_totals(self):
        """محاسبه مجموع قیمت و تعداد آیتم‌ها با تجمیع دیتابیس"""
        # Prevent infinite recursion
        if self._calculating_totals:
            return
        ...
        try:
            ...
            self.total_items = totals['total_items'] or 0  # تنظیم 0 در صورت None
            
            # Use update instead of save to avoid triggering signals and validation
            Order.objects.filter(pk=self.pk).update(
                total_price=self.total_price,
                total_items=self.total_items
            )
        finally:
            self._calculating_totals = False

    def get_total_amount(self):
        return self.total_price + self.shipping_cost + self.tax_amount - self.discount_amount

    def __str__(self):
        return f"Order {self.uid} - {self.customer.user.email} - {self.status}"


class OrderItem(BaseModel):
    order = models.ForeignKey(
        Order, on_delete=models.CASCADE, related_name="orderitems", verbose_name=_("Order")
    )
